...
public class MysqlStoreDao implements IStoreDao {
	private static final Logger logger = Logger.getLogger(MysqlStoreDao.class);

	private DataSource dataSource;

	public MysqlStoreDao(DataSource dataSource) {
	...
	public int insertErrorInfo(ResultInfo resultInfo) {
		...
		try {
			conn = dataSource.getConnection();
			String sql = "insert into `cs_mp_error_info`(`code`,`task_id`,`result_type`,`url_path`,`table_name`,`describe`) values(?,?,?,?,?,?)";
			stat = conn.prepareStatement(sql);
