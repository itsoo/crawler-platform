...
/**
 * 内置采集策略三，通过httpClient处理post请求方式的采集
 *
 * @author a
 */
public class Tactics3 extends Tactics {
    private final static Logger logger = Logger.getLogger(Tactics3.class);

    /**
     * 采集策略的序号标记
     */
    public static final long TACTICS_NUM = 3;
    ...
    public Tactics3(IMQClient mqClient) {
        super(mqClient);
    ...
    public void execute(TaskUnitInfo taskUnit) {
        ...
        // 先访问网路，获取网页源码
        // 先访问网路，获取网页源码
        byte[] result = fetcher.doPostMethod(url, requestHeader);
