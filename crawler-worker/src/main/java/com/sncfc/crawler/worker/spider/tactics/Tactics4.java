...
/**
 * 内置采集策略四，采集结果需要携带源数据的某些字段
 *
 * @author a
 */
public class Tactics4 extends Tactics {
    private final static Logger logger = Logger.getLogger(Tactics4.class);

    /**
     * 采集策略的序号标记
     */
    public static final long TACTICS_NUM = 4;
    ...
    public Tactics4(IMQClient mqClient) {
        super(mqClient);
    ...
    public void execute(TaskUnitInfo taskUnit) {
        ...
        // 先访问网路，获取网页源码
        byte[] result = fetcher.doPostMethod(url);
        ...
        if (result == null) {
            ...
            myDealResultInfo(resultInfo, params);
    ...
    private void myDealResultInfo(ResultInfo resultInfo,
                                  Map<String, String> params) {
        List<Map<String, String>> columns = resultInfo.getColumns();

        for (Map<String, String> column : columns) {
            for (String key : column.keySet()) {
                String paramKey = column.get(key);
                if (paramKey.startsWith("${") && paramKey.endsWith("}")) {
                    String paramValue = params.get(paramKey);
                    column.put(key, paramValue);
                }
            }
        }
